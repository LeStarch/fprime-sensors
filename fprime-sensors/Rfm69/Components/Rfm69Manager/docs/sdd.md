...
| REQ-RFM69MGR-006 | The component shall poll the radio on its `run` port and, when `PayloadReady` or `FifoNotEmpty` is set in receive mode, drain the received packet payload from the FIFO into an allocated buffer and emit it on `dataOut`, remaining in receive mode so packets larger than the FIFO stream through it. | Deadline-driven (rate group) polling of `RegIrqFlags2`; avoids dependence on DIO interrupt lines. Packets up to 255 payload bytes are drained as they arrive per datasheet §5.2.2.3. | Unit test |
